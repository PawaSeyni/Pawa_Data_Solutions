// IndexNow, automatic: tell Bing (and the other IndexNow engines) which pages changed in this deploy.
//
// Runs as the LAST step of the Netlify production build (after the checks, the build and
// the prerender), so a build that fails never announces anything. It fingerprints every sitemap URL by
...
// routes and stray files (iCloud "index 3.html" copies) are never submitted.
// IndexNow is a hint, not an indexing guarantee, and a failure here never fails the build.
//
//   node scripts/indexnow-deploy.mjs        # fingerprint only (no CONTEXT=production)
//   The manual `npm run indexnow` (scripts/indexnow.mjs) stays for a full re-seed with an audit log.
//   CONTEXT=production node scripts/indexnow-deploy.mjs --dry-run   # show what would be sent
