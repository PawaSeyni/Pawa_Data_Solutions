// Post-build hydration check (the last step of `npm run build`, after prerender).
//
// Why: src/main.jsx HYDRATES the prerendered snapshots instead of
// repainting them, which is what removed the mobile layout shift on lazy routes. The
...
// else in the build would notice; only the layout-shift fix is gone.
//
...
const PORT = 5097; // prerender uses 5098
...
// Every route in the sitemap (all four locales).
...
// Plus any prerendered page that is not in the sitemap, and the 404 page, which
// hydrates too. "index 3.html" style names are iCloud conflict copies, never deployed.
...
const SLOW_ROUTES = ['/solutions/data-governance/', '/contact/', '/fr/careers/'].filter(r => routes.includes(r));
...
if (failures.size) {
  ...
  console.error(
    `\n❌ Hydration check failed on ${failures.size} of ${routes.length} routes:\n  ` +
      ...
      'to a full repaint and the layout shift comes back. Usual causes: Math.random()/Date output in\n' +
      'render, or a mount effect that changes visible output. See "Hydration contract" in README.md.\n',
  );
  process.exit(1);
}
console.log(`Hydration check OK: ${routes.length} routes hydrated cleanly (${sitemapRoutes.length} sitemap, ${extraRoutes.length} other, 404; ${SLOW_ROUTES.length} again with slow chunks) in ${secs}s.`);
